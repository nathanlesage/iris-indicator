...
import { compileShader, compileProgram, resizeCanvasToDisplaySize } from "./util/webgl"

import vertexShaderSource from './shaders/vertex_shader.glsl' with { type: 'text' }
import fragmentShaderSource from './shaders/fragment_shader.glsl' with { type: 'text' }
...
export class WebGLEngine {
  ...
  private segmentLocs: Vec4<{ color: WebGLUniformLocation|null, ratio: WebGLUniformLocation|null }>
   ...
   */
  setSegments (segments: Vec4<Segment>) {
    const gl = this.gl
   ...
   * @param {number[]} matrix
   */
  draw (triangleData: Float32Array, count: number, matrix: number[]) {
    const gl = this.gl
    const { cWidth, cHeight } = this.textureSize()
